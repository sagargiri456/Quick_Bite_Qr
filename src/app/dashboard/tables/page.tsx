...
export default function TablesPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-100 p-4 sm:p-8">
            ...
            {tables.map(table => (
              <div key={table.id} className="bg-white p-6 rounded-2xl shadow-md flex flex-col justify-between transition-all hover:shadow-xl hover:-translate-y-1">
                ...
                {table.qr_code_url && table.qr_code_url !== 'generating...' ? (
                  // CHANGED: Pass table_number as the tableName prop
                 ...
                 <button onClick={() => deleteTable(table.id)} className="flex items-center justify-center gap-2 text-red-500 hover:text-red-700 mt-4 text-sm font-medium transition-colors">
                    <Trash2 size={14} />
                    Delete Table
                 </button>
              </div>
