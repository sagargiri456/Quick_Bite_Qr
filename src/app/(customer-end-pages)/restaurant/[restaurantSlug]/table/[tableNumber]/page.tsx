  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans">
      <div className="max-w-7xl mx-auto pb-12">
        <header className="sticky top-0 bg-white/80 backdrop-blur-md z-50 p-2 sm:p-4 shadow-sm">
          <div className="flex justify-between items-center mb-3 sm:mb-4">
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 truncate pr-2">
              {restaurantDetails ? restaurantDetails.restaurant_name : 'Menu'}
            </h1>
            <button
              onClick={() => setIsCartOpen(true)}
              className="relative bg-blue-600 text-white p-2 sm:p-3 rounded-full hover:bg-blue-700 transition-colors flex-shrink-0"
            >
              <ShoppingCart size={18} className="sm:w-5 sm:h-5" />
              {totalItems() > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold rounded-full h-4 w-4 sm:h-5 sm:w-5 flex items-center justify-center animate-bounce text-xs">
                  {totalItems()}
                </span>
              )}
            </button>
                ...
                (category) =>
                  groupedMenu[category] && groupedMenu[category].length > 0 && (
                    <section key={category}>
                      <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-800 capitalize mb-4 sm:mb-6">{category}</h2>
